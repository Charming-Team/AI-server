...
def test_recommendation_service_operator_gets_read_only_urls_without_money_questions() -> None:
    ...
    assert "오늘 먼저 처리해야 할 작업 우선순위를 알려줘" not in questions
    assert "오늘 생산할 제품명과 배정된 생산계획을 조회해줘" not in questions
...
def test_recommendation_service_manager_gets_answerable_manufacturing_questions() -> None:
    service = RecommendationService()
    ...
    assert "오늘 먼저 처리해야 할 작업 우선순위를 알려줘" not in questions
    assert "오늘 생산할 제품명과 배정된 생산계획을 조회해줘" not in questions
    assert "최근 제조 리스크 보고서를 요약해줘" not in questions
