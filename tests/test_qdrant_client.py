...
def test_qdrant_client_scrolls_document_payload_points() -> None:
    captured_request: dict = {}
    ...
    async def run_scroll() -> list[dict]:
        transport = httpx.MockTransport(handler)
        async with httpx.AsyncClient(transport=transport) as http_client:
            client = QdrantDocumentSearchClient(
                Settings(qdrant_api_key="qdrant-token"),
            ...
            return await client.scroll_points(limit=3)

    points = anyio.run(run_scroll)

    assert len(points) == 1
    assert points[0]["id"] == "point-1"
    assert captured_request["url"].endswith(
        "/collections/smap_internal_documents/points/scroll"
    )
    assert captured_request["headers"]["api-key"] == "qdrant-token"
    assert '"limit":3' in captured_request["body"]
    assert '"with_payload":true' in captured_request["body"]
    assert '"with_vector":false' in captured_request["body"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"result": []},
        {"result": {"points": {}}},
        {"result": {"points": ["not-a-dict"]}},
    ],
)
def test_qdrant_client_raises_external_error_on_invalid_scroll_shape(
    body: object,
) -> None:
    client = QdrantDocumentSearchClient(Settings())
    response = httpx.Response(
        200,
        request=httpx.Request("POST", "http://qdrant.local/scroll"),
        json=body,
    )

    with pytest.raises(ChatExternalServiceError) as exc_info:
        client._parse_scroll_response(response)
