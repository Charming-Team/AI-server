...
risk_slots = [
    {
        ...
        "body": "주요 지연 원인은 자재 부족과 라인 상태 이상이 동시에 발생한 복합 요인입니다. 일부 필요 자재가 부족하며, 배정 라인의 생산 부하도 높은 상태입니다. 자재가 확보되더라도 현재 라인 일정이 납기 이후까지 이어질 가능성이 있어 단순 자재 보충만으로는 납기 리스크를 해소하기 어렵습니다.",
        "action": "부족 자재 확보 가능 시점을 우선 확인하고, 동시에 일부 물량을 대체 라인으로 분산하십시오. 납기 우선 주문으로 분류하여 생산계획 수정 화면에서 라인 재배정 또는 계획 순서 조정을 우선 적용하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.2840,
        ...
        "body": "주요 지연 원인은 자재 부족입니다. 현재 생산계획 기준 일부 첨가제와 포장재의 예약 수량이 필요 수량보다 부족해 생산 시작 시점에 전체 투입량을 확보하지 못할 가능성이 있습니다.",
        "action": "부족 자재의 가용 재고와 입고 예정량을 확인하고, 동일 자재를 사용하는 후순위 계획의 예약 수량 조정을 검토하십시오.",
    },
    {
        "level": "WARNING",
        "prob": 0.5270,
        ...
        "body": "주요 지연 원인은 자재 입고 지연입니다. 생산에 필요한 원료의 입고 예정일이 생산 필요 시점보다 늦어, 현재 일정대로는 생산 착수가 지연될 가능성이 높습니다.",
        "action": "입고 예정 자재의 조기 입고 가능 여부를 확인하고, 입고 전까지 동일 라인에 배정 가능한 다른 주문을 선행 배치하는 방안을 검토하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.7531,
        ...
        "body": "주요 지연 원인은 라인 상태 이상입니다. 배정 라인의 가동률이 높고 대기시간이 증가한 상태이며, 현재 주문 수량 대비 예상 소요시간이 납기 여유를 초과하고 있습니다.",
        "action": "동일 제품 생산이 가능한 대체 라인의 여유 시간을 확인하고, 납기 우선순위가 낮은 계획을 후순위로 이동하는 방안을 검토하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.3360,
        "delay_days": 0.8,
        "causes": ["LOW_YIELD"],
        "body": "주요 지연 원인은 수율 저하입니다. 해당 제품-라인 조합의 기준 수율 대비 예상 수율이 낮아, 동일 주문량을 충족하기 위한 추가 생산 시간이 필요할 수 있습니다.",
        "action": "해당 제품의 최근 생산 실적과 불량 발생량을 확인하고, 계획 수량에 여유분을 반영하거나 수율이 높은 라인으로 재배정하는 방안을 검토하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.8124,
        ...
        "body": "주요 지연 원인은 설비 상태 이상입니다. 일부 Machine의 점검 또는 정지 상태가 확인되어 생산 흐름이 중단될 가능성이 큽니다.",
        "action": "설비 이상이 있는 Machine의 복구 일정을 확인하고, 동일 제품을 처리할 수 있는 대체 설비 또는 대체 라인 투입 가능 여부를 검토하십시오.",
    },
    {
        "level": "WARNING",
        "prob": 0.6120,
        ...
        "body": "설비 상태 이상과 라인 부하가 동시에 나타나고 있습니다. 병목 설비가 정상화되지 않으면 후속 공정 대기시간도 증가할 가능성이 있습니다.",
        "action": "설비 점검 완료 예상 시각을 확인하고, 병목 설비 이후 공정의 대기 작업량을 줄이도록 생산 순서를 조정하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.1860,
        "delay_days": 0.4,
        "causes": ["LINE_ABNORMAL"],
        "body": "즉시 계획 변경이 필요한 수준은 아니지만, 라인 대기시간이 증가하고 있어 추적이 필요합니다. 신규 긴급 주문이 추가되면 지연 위험이 높아질 수 있습니다.",
        "action": "현재 계획은 유지하되 해당 라인의 대기시간과 가동률을 모니터링하십시오. 신규 긴급 주문 발생 시 납기 여유를 재계산하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.7352,
        "delay_days": 8.3,
        "causes": ["MATERIAL_SHORTAGE", "LINE_ABNORMAL"],
        "body": "자재 부족과 라인 부하가 동시에 나타나 현재 생산계획 유지 시 납기 내 완료 가능성이 낮습니다.",
        "action": "자재 확보 일정과 대체 라인 투입 가능성을 동시에 확인하고, 분할 생산 또는 라인 재배정을 검토하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.2270,
        "delay_days": 1.1,
        "causes": ["MATERIAL_DELAY"],
        "body": "주요 지연 원인은 자재 입고 지연입니다. 자재 수량은 확보 가능하지만 투입 가능 시점이 늦어지는 것이 핵심 리스크입니다.",
        "action": "입고 예정 자재의 조기 입고 가능 여부를 확인하고, 입고 전까지 선행 가능한 계획을 우선 배치하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.9135,
        ...
        "body": "예상 지연 일수는 크지 않지만, 자재 투입 시점과 라인 부하가 동시에 불안정해 지연 발생 확률이 매우 높습니다.",
        "action": "자재 투입 가능 시각을 확정하고, 같은 라인에 배정된 후속 계획의 순서를 조정해 짧은 지연이 확산되지 않도록 관리하십시오.",
    },
    {
        "level": "WARNING",
        "prob": 0.4890,
        ...
        "body": "주요 지연 원인은 수율 저하와 설비 상태 이상입니다. 최근 동일 제품 생산에서 수율이 기준보다 낮고, 일부 설비 상태도 정상 가동 상태가 아닙니다.",
        "action": "설비 복구 일정을 확인한 뒤 예상 수율을 반영하여 계획 수량을 보정하십시오. 필요 시 안정 라인으로 일부 물량 이전을 검토하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.7818,
        "delay_days": 11.2,
        "causes": ["LINE_ABNORMAL"],
        "body": "배정 라인의 작업량이 집중되어 납기 여유 대비 생산 소요시간이 과도합니다. 현재 계획을 유지하면 납기 내 완료 가능성이 낮습니다.",
        "action": "생산계획 수정 화면에서 해당 주문의 라인 재배정 가능 여부를 확인하고, 후순위 주문의 시작 시각 조정을 검토하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.3190,
        "delay_days": 2.0,
        "causes": ["LOW_YIELD"],
        "body": "수율 저하로 인해 주문 수량 충족을 위한 추가 생산 시간이 필요할 수 있습니다. 납기 여유가 충분하지 않으므로 추적이 필요합니다.",
        "action": "예상 불량량을 반영하여 계획 수량을 보정하고, 수율이 높은 라인으로 일부 물량 이동을 검토하십시오.",
    },
    {
        "level": "WARNING",
        "prob": 0.5810,
        "delay_days": 2.8,
        "causes": ["LINE_ABNORMAL"],
        "body": "납기 여유 대비 생산 소요시간이 높고 배정 라인의 가동률이 높아 생산 흐름 지연 가능성이 있습니다.",
    ...
    },
    {
        ...
        "delay_days": 1.4,
        "causes": ["MACHINE_ABNORMAL"],
        "body": "설비 이상으로 인한 생산 흐름 중단 가능성이 높습니다. 예상 지연 일수는 제한적이지만 지연 발생 확률이 높아 즉시 확인이 필요합니다.",
        "action": "설비 담당자에게 복구 예상 시간을 확인하고, 해당 공정의 우회 생산 가능 여부를 검토하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.1280,
        "delay_days": 0.2,
        "causes": ["LINE_ABNORMAL"],
        "body": "라인 대기시간 증가가 관찰되지만 현재는 납기 여유가 남아 있습니다. 다만 후속 주문 추가 시 위험이 상승할 수 있습니다.",
    ...
    },
    {
        ...
        "action": "부족 자재의 대체 조달 가능성을 확인하고, 확보 시점에 맞춰 생산 시작 시각을 재조정하십시오.",
    },
    {
        "level": "WARNING",
        "prob": 0.6470,
        ...
        "body": "자재 투입 시점 지연과 예상 수율 저하가 함께 나타나 생산 수량 충족에 불확실성이 있습니다.",
        "action": "입고 일정과 예상 수율을 동시에 반영하여 계획 수량과 시작 시각을 재검토하십시오.",
    },
    {
        "level": "CAUTION",
        "prob": 0.3650,
        "delay_days": 1.6,
        "causes": ["MACHINE_ABNORMAL"],
        "body": "일부 설비 상태 이상이 생산 흐름에 영향을 줄 수 있습니다. 현재는 즉시 지연 확정 단계는 아니지만 점검이 필요합니다.",
        "action": "설비 상태를 재확인하고, 이상 지속 시 안정 라인으로 일부 물량 이전을 검토하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.8891,
        ...
        "body": "라인 부하와 수율 저하가 동시에 나타나 현재 계획 유지 시 납기 내 완료 가능성이 매우 낮습니다.",
    ...
    },
    {
        ...
        "delay_days": 0.5,
        "causes": ["MATERIAL_DELAY"],
        "body": "자재 입고 일정이 생산 필요 시점과 가까워 추적이 필요합니다. 입고가 하루만 늦어져도 생산 착수가 지연될 수 있습니다.",
    ...
    },
    {
        ...
        "body": "자재 부족과 설비 상태 이상이 동시에 발생해 현재 생산계획 유지 시 장기 지연 가능성이 높습니다.",
        "action": "자재 확보와 설비 복구 계획을 동시에 수립하고, 생산계획 수정 화면에서 대체 라인 분산안을 우선 적용하십시오.",
    },
    {
        "level": "CRITICAL",
        "prob": 0.9284,
        "delay_days": 0.6,
        "causes": ["LINE_ABNORMAL"],
        "body": "예상 지연 일수는 짧지만 라인 부하와 순서 제약으로 인해 지연 발생 확률이 매우 높습니다. 단기 대응을 놓치면 후속 주문까지 영향이 확산될 수 있습니다.",
        "action": "해당 주문을 납기 우선순위로 올리고, 같은 라인의 후속 계획 순서를 즉시 조정하십시오.",
    },
]
...
def apply_source_mutation(conn, row: dict[str, Any], slot: dict[str, Any]) -> None:
    ...
    if row.get("plan_id") is not None:
        ...
        # 단, 실제 UI의 생산 진행률은 production_results 기준이라 이 업데이트만으로 진행률이 바뀌지는 않습니다.
        conn.execute(
            text("""
...
def main() -> None:
    with engine.begin() as conn:
        ...
        if len(eligible_rows) < len(risk_slots):
            raise RuntimeError(
                f"데모 seed 대상 미완료 주문이 부족합니다. 필요={len(risk_slots)}, 현재={len(eligible_rows)}"
            )

        # 기존 데모 seed 제거. 원래 모델 row는 건드리지 않습니다.
        old_demo_ids = conn.execute(
            text("""
