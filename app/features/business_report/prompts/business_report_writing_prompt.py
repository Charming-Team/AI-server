...
3. 기술적인 내용을 제거하지 말고, 경영진이 이해하기 쉬운 업무 표현으로 충분히 재작성합니다. 원문이 이미 보고서체이더라도 표현, 용어, 문장 톤은 경영진 보고서에 맞게 한 단계 더 정리합니다.
4. report_content는 문자열이 아니라 기존과 동일한 json 객체 형태를 유지해야 합니다.
5. report_content 내부의 toc, format, version, sections, report_kind 등 메타 구조와 섹션 구성은 유지하되, 본문 문장은 원문이 이미 보고서체인 경우에도 표현과 용어를 더 간결하고 공식적인 경영진 보고 문체로 재작성합니다.
6. report_content 안에 sections 배열이 있는 경우, 각 section의 content를 포함한 보고서 전체 내용을 경영진 보고 문체로 변환합니다.
7. 영어 섹션명, 시스템 용어, 상태값은 의미를 유지하는 범위에서 자연스러운 한국어 보고 표현으로 우선 변환합니다. 특별한 이유가 없는 한 원문 영어 표현을 그대로 유지하지 않습니다. 필요 시에만 한국어 표현 뒤에 원문을 괄호로 병기합니다.
...
16. report_content 본문은 원문을 단순 복사하거나 최소한으로만 수정해서는 안 됩니다. 의미와 수치는 유지하되, 문장 표현과 용어는 원문과 분명히 구별될 정도로 경영진 보고 문체로 재작성해야 합니다.
