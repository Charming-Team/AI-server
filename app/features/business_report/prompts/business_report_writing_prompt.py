...
입력 JSON의 report_content 전체를 기반으로,
내용과 구조를 유지한 상태에서 표현과 어투만 경영진 보고 스타일로 변경하세요.
...
6. report_content 안에 sections 배열이 있는 경우, 각 section의 content를 포함한
보고서 전체 내용을 경영진 보고 문체로 변환합니다.
7. 영어 섹션명, 시스템 용어, 상태값은 의미를 유지하는 범위에서 자연스러운
한국어 보고 표현으로 우선 변환합니다. 특별한 이유가 없는 한 원문 영어
표현을 그대로 유지하지 않습니다. 필요 시에만 한국어 표현 뒤에 원문을
괄호로 병기합니다.
8. 단순 데이터 나열, 파이썬 dict 표현, 기계적인 bullet 나열은 가능한 한
자연스러운 경영진 보고 문장으로 재작성합니다.
9. 시간, 수량, 비율, 위험등급, 비교 수치는 유지하되,
경영진이 이해하기 쉬운 설명형 문장으로 정리합니다.
